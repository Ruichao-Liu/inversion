...
class NullInversion:
    # 计算扩散过程中的前一步样本。通过当前时间步和模型输出，推导出前一时间步的样本。
    ...
    # 计算扩散过程中的下一步样本。与 prev_step 类似，但方向相反
    ...
    # 获取给定潜在变量、时间步和上下文的噪声预测。
    def get_noise_pred_single(self, latents, t, context):
        noise_pred = self.model.unet(latents, t, encoder_hidden_states=context)["sample"]
        return noise_pred

    # 获取噪声预测并执行下一步或上一步扩散过程，基于是否是正向过程。
    ...
    def init_prompt(self, prompt: str):
        ...
        # 将有条件和无条件embedding进行拼接
        self.context = torch.cat([uncond_embeddings, text_embeddings])
        self.prompt = prompt

    # 进行 DDIM 正向扩散过程，生成一系列潜在变量。
    @torch.no_grad()
    ...
    # 进行图像的 DDIM 反向扩散过程，将图像转换为潜在表示并重构图像
    @torch.no_grad()
