...
# ---------------- Global parameter ----------------
MODEL_PATH = "/home/lrc/.cache/huggingface/hub/models--CompVis--stable-diffusion-v1-4/snapshots/133a221b8aa7292a167afc5127cb63fb5005638b"
LOW_RESOURCE = True
NUM_DIFFUSION_STEPS = 50  # diffusion步数
GUIDANCE_SCALE = 7.5
MAX_NUM_WORDS = 77
DEFAULT_IMG_HEIGHT = 512  # 默认图片高
DEFAULT_IMG_WIDTH = 512  # 默认图片宽
...
class EditParams:
    source_prompt: str
    target_prompt: str
    cross_merge_end_step: Union[
        float, int]  # cross attention merge step, 0-(cross_merge_step * diffusion step)  using cross-attn injection
    self_merge_end_step: Union[
        float, int]  # self attention merge step, 0-(self_merge_step * diffusion step) using self-attn injection
    cross_merge_start_step: Union[
        float, int] = 0  # cross attention merge step, 0-(cross_merge_step * diffusion step)  using cross-attn injection
    self_merge_start_step: Union[
        float, int] = 0  # self attention merge step, 0-(self_merge_step * diffusion step) using self-attn injection
...
class LocalBlend:
    ...
    def __call__(
        ...
        mask = F.max_pool2d(mask, (self.k * 2 + 1, self.k * 2 + 1), (1, 1),
                            padding=(self.k, self.k))  # 2, 1, 16, 16, 相当于做一个膨胀（取大的值）
    ...
    def get_aim_map(self, batch_size: int, attn_map: Dict[str, Dict[str, CachedAttnInfo]]) -> torch.Tensor:
        ...
        maps = [item.reshape(batch_size, -1, 1, 16, 16, MAX_NUM_WORDS) for item in
                valid_map]  # b * head * 1 * h * w * dim
...
# 在diffsuion的基础上重写cross attention
...
# 找到所有CrossAttention层，并替换为control_cross_attn_forward前向传播
def register_attention_control_mine(unet, controller):
    # 用于存储cross attention的名称
    cross_attn_name_ls = []
    # 遍历模型的子模块
    for i in unet.named_children():
        name, cur_module = i[:2]
        # 是否为CrossAttention，如果是则用自定义forward,并将名字添加至列表
...
class BaseDiffusionModel:
    ...
    # 模拟变分自编码器（VAE）编码器的输出，该输出会被下采样8倍
    def init_latent(
            self,
            batch_size: int,
        ...
        latents = latent.expand(batch_size, self.unet.in_channels, height // 8, width // 8).to(device=self.device,
                                                                                               dtype=TORCH_DTYPE)
    ...
    def get_text_embed(self, prompt: List[str]) -> torch.Tensor:
        ...
        text_embeddings: torch.Tensor = self.text_encoder(text_token_id)[0]  # (2,77,768)
    ...
    @torch.no_grad()
    def diffusion_step(self, latents, context, t, guidance_scale=GUIDANCE_SCALE,
                       low_resource=True) -> torch.FloatTensor:
        """
        diffusion step
        Args:
            latents: torch.Tensor, shape: (batch_size, in_channels, height // 8, width // 8)
...
class Prompt2Prompt(BaseDiffusionModel):
    ...
    # 对于一个prompt,将其生成两个Embedding,一个是unconditional embedding,另一个是prompt embedding。
    # 对于任意一个prompt,unconditional embedding都一样
    def get_context_embed(self, prompt_ls: List[str]) -> List[torch.Tensor]:
        ...
        Returns:
            ...
            the second item is prompt embedding
        """
        prompt_embed = self.get_text_embed(prompt_ls)  # (2,77,768)
    ...
    # 控制条件下的text2image（结合了源图的生成结果，扩散步骤中控制注意力层）
    @torch.no_grad()
...
if __name__ == "__main__":
    ...
    source_image = res.source_image
    latent = res.latent
    latent = transforms.ToPILImage()(latent.squeeze(0))

    img = Image.fromarray(target_image).convert("RGB")
    img.show()
